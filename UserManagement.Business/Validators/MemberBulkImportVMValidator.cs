...
namespace UserManagement.Business.Validators
{
    public class MemberBulkImportVMValidator : AbstractValidator<MemberBulkImportVM>
    {
        public MemberBulkImportVMValidator()
        {
            ...
            RuleFor(x => x.UserEmail).EmailAddress().When(x=>!string.IsNullOrWhiteSpace(x.UserEmail)).WithMessage("Invalid User Email !");
            RuleFor(x => x.UserMobile).Matches("^[0-9]{10}$").When(x=>!string.IsNullOrWhiteSpace(x.UserMobile)).WithMessage("Invalid User Mobile !");
            RuleFor(x => x.StateId).NotEmpty().WithMessage("Invalid HF State Name !");
            RuleFor(x => x.DistrictId).NotEmpty().WithMessage("Invalid HF District Name !");
            RuleFor(x => x.CityId).NotEmpty().WithMessage("Invalid HF City Name !");
