...
namespace UserManagement.Business
{
    public class MemberBulkDataImportService : IBulkDataImportService<MemberBulkImportVM>
    {
        ...
        public string GetUsersName(IEnumerable<StateDistrictCity> states, string StateName, string DistrictName, string HFName, string Type)
        {
            ...
            if (string.IsNullOrEmpty(DistShortCode) || DistShortCode.Length < 2)
            {
        ...
        {
            return states.Where(x => x.StateName == stateName)
        ...
        {
            return states.Where(x => x.StateName == stateName && x.DistrictName == districtName)
        ...
        {
            return states
                .Where(x => x.StateName == stateName && x.DistrictName == districtName && x.CityName == cityName)
        ...
        {
            return states
                .Where(x => x.StateName == stateName && x.DistrictName == districtName)
        ...
        {
            return Convert.ToString(institutions
        ...
        private async Task<IEnumerable<ResultModel<MemberBulkImportVM>>> CreateServiceProvider(IEnumerable<ResultModel<MemberBulkImportVM>> models)
        {
            ...
            var institutes = models.Where(x => x.Model.InstituteID == string.Empty)
            ...
            if (institutes != null && institutes.Count() > 0)
            {
                ...
                var csvUtility = new CsvUtility<InstitutionModel>();
                var stream = csvUtility.Write(institutes);
                var records = await bulkInsertRepository.BulkInsertInstitution(stream);
                var results = await bulkInsertRepository.GetInstituations(maxInstituteId + 1, maxInstituteId + records);
        ...
        {
            var modelReturns = models;
            var members = models.Select(x => new MembersModel()
                 {
            ...
            if (members != null && members.Count() > 0)
            {
                ...
                var csvUtility = new CsvUtility<MembersModel>();
                var stream = csvUtility.Write(members);
                var records = await bulkInsertRepository.BulkInsertInstitution(stream);
                var results = await bulkInsertRepository.GetMembers(maxMemberId + 1, maxMemberId + records);
        ...
        {
            var modelReturns = models;
            var logins = models.Select(x => new LoginModel()
            {
            ...
            {
                var csvUtility = new CsvUtility<LoginModel>();
        ...
        {
            var modelReturns = models;
            var memberSlots = models.Select(x => new MemberSlotModel()
            {
            ...
            {
                var csvUtility = new CsvUtility<MemberSlotModel>();
        ...
        private async Task<IEnumerable<ResultModel<MemberBulkImportVM>>> CreateMemberInstitution(IEnumerable<ResultModel<MemberBulkImportVM>> models)
        {
            ...
            {
                var csvUtility = new CsvUtility<MemberInstitutionModel>();
