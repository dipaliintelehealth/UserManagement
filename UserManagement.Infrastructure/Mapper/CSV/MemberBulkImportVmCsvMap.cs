    ...
    public class MemberBulkImportVmCsvMap : ClassMap<MemberBulkImportVM>
    {
        public MemberBulkImportVmCsvMap()
        {
           ...
           // Map(m => m.AssignHF).Index(31);
            ...
            Map(m => m.GenderId).Index(37);
            Map(m => m.HFTypeId).Index(38);
           // Map(m => m.AssignedInstituteID).Index(41);
            Map(m => m.InstituteID).Index(39);
            Map(m => m.MemberId).Index(40);
            Map(m => m.UserDistrictShortCode).Index(41);
            Map(m => m.UserName).Index(42);
            Map(m => m.HFNameWithDistrictName).Index(43);
            Map(m => m.SubMenuName).Index(44);
            Map(m => m.SpecialityId).Index(45);
            Map(m => m.SelectedSpecialityId).Index(46);
